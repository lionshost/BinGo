...
const routeFocusBar =
    document.getElementById("routeFocusBar");

const focusBadge =
    document.getElementById("focusBadge");

const focusTruckTitle =
    document.getElementById("focusTruckTitle");

const focusTruckSub =
    document.getElementById("focusTruckSub");

const toggleTimelineBtn =
    document.getElementById("toggleTimelineBtn");

const followTruckBtn =
    document.getElementById("followTruckBtn");

const exitFocusBtn =
    document.getElementById("exitFocusBtn");

const timelineDrawer =
    document.getElementById("timelineDrawer");

const drawerBadge =
    document.getElementById("drawerBadge");

const drawerTruckTitle =
    document.getElementById("drawerTruckTitle");

const drawerProgressText =
    document.getElementById("drawerProgressText");

const drawerProgressBar =
    document.getElementById("drawerProgressBar");

const timelineStopsList =
    document.getElementById("timelineStopsList");

const closeDrawerBtn =
    document.getElementById("closeDrawerBtn");

const focusOnlyModeCheckbox =
    document.getElementById("focusOnlyMode");
...
function createBinIcon(item) {
    ...
    if (selectedTruckId) {
        const selTruck = trucks.find(t => t.id === selectedTruckId);
        if (selTruck) {
...
function refreshBinVisibility() {
    ...
    if (selectedTruckId && focusOnlyMode) {
        const selTruck = trucks.find(t => t.id === selectedTruckId);
        const assignedSet = new Set((selTruck?.orderedBins || []).map(b => b.id));
        bins.forEach(item => {
            const marker = binMarkers.get(item.id);
            if (!marker) return;
        ...
        if (visibleCount) visibleCount.textContent = assignedSet.size;
...
function updateRouteVisibility() {
    ...
    routeLayers.forEach(
        line => {
            ...
            if (selectedTruckId) {
                if (line.truck.id === selectedTruckId) {
...
function updateTruckMarkerVisual(truck, marker) {
    ...
    if (container) {
        ...
        container.classList.toggle("selected-truck", selectedTruckId === truck.id);
    }
    if (selectedTruckId === truck.id && followSelectedTruck && typeof map.panTo === "function") {
...
function moveTruck(truck, marker, meters = TRUCK_SPEED_KMH / 3.6 * speedMultiplier) {
    ...
    if (truck.dwellRemaining > 0 && meters > 0) {
        truck.dwellRemaining = Math.max(0, truck.dwellRemaining - (0.1 * speedMultiplier));
        updateTruckMarkerVisual(truck, marker);
        if (selectedTruckId === truck.id) updateTimelineUI();
        return;
    }
    ...
    if (movement.reached && movement.reached.length > 0) {
        ...
        if (meters > 0 && speedMultiplier < 10) {
...
function selectTruck(truck, userInitiated = true) {
    selectedTruckId = truck ? truck.id : null;

    truckCards.forEach((item, id) => {
        item.classList.toggle("selected", id === selectedTruckId);
    });

    if (truck) {
        if (routeFocusBar) {
            routeFocusBar.hidden = false;
            if (focusBadge) {
                focusBadge.textContent = truck.id;
                focusBadge.style.background = truck.color;
            }
            if (focusTruckTitle) focusTruckTitle.textContent = truck.name;
            if (focusTruckSub) {
                const districtStr = truck.district ? `${truck.district} · ` : '';
                focusTruckSub.textContent = `${districtStr}${truck.assignedBins.length} trạm thu gom`;
            }
        }
        if (timelineDrawer) {
            timelineDrawer.hidden = false;
            if (drawerBadge) {
                drawerBadge.textContent = truck.id;
                drawerBadge.style.background = truck.color;
            }
            if (drawerTruckTitle) drawerTruckTitle.textContent = truck.name;
        }
        updateTimelineUI();

        if (userInitiated && truck.route && truck.route.length >= 2 && typeof map.fitBounds === "function") {
            try {
        ...
        if (routeFocusBar) routeFocusBar.hidden = true;
        if (timelineDrawer) timelineDrawer.hidden = true;
        followSelectedTruck = false;
        if (followTruckBtn) followTruckBtn.classList.remove("active");
    }
...
function updateTimelineUI() {
    if (!selectedTruckId || !timelineDrawer || timelineDrawer.hidden) return;
    ...
    if (drawerProgressText) {
        ...
        drawerProgressText.textContent = statusText;
    }
    if (drawerProgressBar) {
        drawerProgressBar.style.width = `${percent}%`;
        drawerProgressBar.style.background = truck.color;
    }

    if (!timelineStopsList) return;
    ...
    truck.orderedBins.forEach((bin, idx) => {
        ...
        row.addEventListener("click", () => {
            if (bin.latitude && bin.longitude && typeof map.setView === "function") {
                map.setView([bin.latitude, bin.longitude], 17);
                const marker = binMarkers.get(bin.id);
                if (marker && typeof marker.openPopup === "function") marker.openPopup();
            }
        });

        timelineStopsList.appendChild(row);
    });
}
...
function renderTruckList() {
    ...
    trucks.forEach(truck => {
        ...
        item.className = `truck-item ${truck.id === selectedTruckId ? 'selected' : ''}`;
        item.tabIndex = 0;
        ...
        truckList.appendChild(item);
            ...
            item.classList.toggle("selected", truck.id === selectedTruckId);
        }
...
if (exitFocusBtn) {
    exitFocusBtn.addEventListener("click", () => selectTruck(null));
}
if (toggleTimelineBtn) {
    toggleTimelineBtn.addEventListener("click", () => {
        if (timelineDrawer) timelineDrawer.hidden = !timelineDrawer.hidden;
    });
}
if (closeDrawerBtn) {
...
if (followTruckBtn) {
    followTruckBtn.addEventListener("click", () => {
        followSelectedTruck = !followSelectedTruck;
        followTruckBtn.classList.toggle("active", followSelectedTruck);
    });
}
if (focusOnlyModeCheckbox) {
    focusOnlyModeCheckbox.addEventListener("change", () => {
        focusOnlyMode = focusOnlyModeCheckbox.checked;
        refreshBinVisibility();
        updateRouteVisibility();
    });
}
